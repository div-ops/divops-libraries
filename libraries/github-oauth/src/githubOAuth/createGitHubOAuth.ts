...
export const createGitHubOAuth = ({
  ...
  return {
    ...
    createResource: async <S, R>({
      ...
      console.log(`[go.createResource] 1 STEP: ${Date.now() - start}ms`);
      ...
      console.log(`[go.createResource] 2 STEP: ${Date.now() - start}ms`);

      if (keyId == null) {
        console.log(`[go.createResource] 3 STEP: ${Date.now() - start}ms`);
        keyId = await (async () => {
      ...
      console.log(`[go.createResource] 4 STEP: ${Date.now() - start}ms`);
    ...
    updateResource: async <S, R>({
      ...
      const start = Date.now();
      console.log(`[go.ureateResource] 1 STEP: ${Date.now() - start}ms`);
      const resourceListKey = `gist-storage-${server}-${client}-${model}-${githubId}`;

      const keyId = await gistStorage.getId(resourceListKey);
      console.log(`[go.ureateResource] 2 STEP: ${Date.now() - start}ms`);

      const prevList = await gistStorage.getById<any>(keyId);
      console.log(`[go.ureateResource] 3 STEP: ${Date.now() - start}ms`);
      ...
      console.log(`[go.ureateResource] 4 STEP: ${Date.now() - start}ms`);

      await gistStorage.setById(id, updatedItem);
      console.log(`[go.ureateResource] 5 STEP: ${Date.now() - start}ms`);
      ...
      console.log(`[go.ureateResource] 6 STEP: ${Date.now() - start}ms`);
    },
  };
};
